...
def initialize_distill_torch(model, dataset, tokenizer, cfg_train, cfg_impl, elapsed_time, setup=_default_setup):
...
class DistillTorchEngineMinimal(torch.nn.Module):
    ...
    def step(self, batch: dict[str, torch.Tensor], pretrain=True):
        ...
        with context():
            ...
            if not pretrain: # Handling the finetuning call
                outputs = self.forward(**batch, compute_distillation=False)
                loss = outputs["loss"]
                self.backward(loss)
                self.optimizer_step()
                return loss.detach()
