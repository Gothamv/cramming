...
def main_downstream_process(cfg, setup):
    ...
    cfg_arch.student_output = True if cfg.impl.student_output else False
    cfg_arch.load_student_layers = cfg.impl.load_student_layers
    ...
    for task_name, task in tasks.items():
        ...
        for epoch in range(cfg.eval.epochs):
            ...
            for step, batch in enumerate(task["trainloader"]):
                ...
                loss = model_engine.step(device_batch, pretrain=False)
        ...
        # Log quantized model metrics
        log.info(f"Quantized model validation metrics: {metrics[task_name + '_quantized']}")
        cramming.utils.wandb_log({f"{task_name}_quantized_{k}": [v] for k, v in metrics[task_name + '_quantized'].items()}, cfg)
...
def validate_quantized(quantized_model, validloader, metric, setup, cfg):
    """Evaluate on validation set using the quantized model."""
    quantized_model.eval()
    for step, batch in enumerate(validloader):
        device_batch = {k: v.to(setup["device"]) for k, v in batch.items() if k in ["input_ids", "labels", "attention_mask"]}
        outputs = quantized_model(**device_batch)
